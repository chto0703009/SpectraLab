function tests = test_analysis_statusMDensity
%TEST_ANALYSIS_STATUSMDENSITY Tests for Status M RGB density analysis.
...
function testReturnsExpectedFields(testCase)
    ...
    result = spectralab.analysis.statusMDensity( ...
        reference, ...
        sample);
    ...
    verifyEqual(testCase, result.Standard, "Status M");
end
...
% Status M channel, regardless of the shape of the weighting functions.
...
function testZeroTransmissionProducesInfiniteDensity(testCase)
	...
	function testSampleAboveReferenceProducesWarning(testCase)
	    ...
	    verifyWarning(testCase, ...
	        @() spectralab.analysis.statusMDensity(reference, sample), ...
