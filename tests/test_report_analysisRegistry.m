...
for analysisId = ["ANL-SPECTRUM", "ANL-CRI"]
    ...
    titleTopInFigure = ax.Position(2) + ...
        (titleExtent(2) + titleExtent(4)) * ax.Position(4);
...
verifyEqual(testCase, ...
    [definition.ResultFields.Field], ...
    ["SourceAFile", "SourceBFile", "SampleCount", "WavelengthMinimum", ...
     "WavelengthMaximum", "MeanTransmissionPercent"]);
end
...
verifyEqual(testCase, result.MeanTransmissionPercent, 50, AbsTol=1e-12);
verifyEqual(testCase, result.SourceAFile, "Reference");
verifyEqual(testCase, result.SourceBFile, "Sample");
verifyTrue(testCase, isfield(result, "TransmissionAnalysis"));
end

function testTransmissionFigureIdentifiesBothSources(testCase)
entry = findEntry(spectralab.report.internal.createAnalysisRegistry(), ...
    "ANL-001");
reference = makeTransmissionArchive(ones(36,1), "Reference");
sample = makeTransmissionArchive(0.5 .* ones(36,1), "Sample");
result = entry.AnalysisRunner(reference, sample);
...
legendHandle = findall(fig, "Type", "legend");
verifyNumElements(testCase, legendHandle, 1);
verifyTrue(testCase, contains(string(legendHandle.String), ...
    "Sample / Reference"));
panelText = findall(fig, "Tag", "SpectraLabTransmissionSummary");
verifyNumElements(testCase, panelText, 1);
verifyTrue(testCase, contains(join(string(panelText.String), newline), ...
    "Reference" + newline + "Reference"));
verifyTrue(testCase, contains(join(string(panelText.String), newline), ...
    "Sample" + newline + "Sample"));
end
...
verifyEqual(testCase, ...
    ...
    ["RedDensity", "GreenDensity", "BlueDensity", ...
     "WhiteDensity", ...
     "RedTransmittancePercent", "GreenTransmittancePercent", ...
     "BlueTransmittancePercent", "WhiteTransmittancePercent"]);
verifyEqual(testCase, ...
    string({definition.ResultFields(5:8).Format}), ...
    repmat("%.2f", 1, 4));
verifyEqual(testCase, ...
    string({definition.ResultFields(5:8).Unit}), ...
    repmat("%", 1, 4));
end
...
verifyEqual(testCase, ...
    ...
    AbsTol=1e-12);
verifyEqual(testCase, ...
    [result.RedTransmittancePercent, ...
     result.GreenTransmittancePercent, ...
     result.BlueTransmittancePercent, ...
     result.WhiteTransmittancePercent], ...
...
verifyEqual(testCase, ...
    [definition.ResultFields.Field], ...
    ["SourceAFile", "SourceBFile", "Density", "Transmittance"]);
end
...
verifyTrue(testCase, contains(string(legendHandle.String), ...
    "Sample / Reference"));
panelText = findall(fig, "Tag", ...
    "SpectraLabIsoVisualDensitySummary");
verifyNumElements(testCase, panelText, 1);
verifyTrue(testCase, contains(join(string(panelText.String), newline), ...
    "Reference" + newline + "Reference"));
verifyTrue(testCase, contains(join(string(panelText.String), newline), ...
