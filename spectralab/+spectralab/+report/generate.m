...
[~, reportName, reportExtension] = fileparts(pdfFile);
context.Report.Filename = string(reportName) + string(reportExtension);
...
if definition.HasFigure && options.ExportPNG
    ...
    if definition.AnalysisId == "ANL-SPECTRUM" && options.PNGInformation
    ...
    elseif hasSpectrumAnalysisSummary(result)
        spectralab.plot.spectrumAnalysisPanel( ...
            pngRenderContext.Graphics.Axes, result);
    end
...
function value = hasSpectrumAnalysisSummary(result)
