...
%
% Generate the registered ANL-008 Status M RGB and ISO visual density PDF
% for the bundled synthetic reference and sample archives. ANL-008 has no
...
outputBase = "example_sample_a_StatusM";
pdfFile = fullfile(reportFolder, outputBase + "_ANL-008_report.pdf");
if isfile(pdfFile)
...
reportInfo = spectralab.report.generate( ...
    [referenceFile, sampleFile], "ANL-008", reportFolder, ...
    OutputBaseName=outputBase, OpenPDF=false);
fprintf("SpectraLab Status M report created:\n  %s\n", ...
    reportInfo.PDFFile);
