...
action = questdlg( ...
    "Correct patches in a COMPLETED ColorChecker session, or resume an unfinished correction?", ...
...
if action == "New correction"
    ...
    if any(string({source.Patches.State}) ~= "measured")
        error("SpectraLab:Examples:ColorCheckerSessionIncomplete", ...
            ["This ColorChecker acquisition is paused and is not ready " ...
             "for controlled remeasurement.\n\nResume the acquisition with " ...
             "the ColorChecker measurement workflow. Use this example " ...
             "only after every patch has been measured."]);
    end
    ...
    [file, folder] = uigetfile( ...
        {"*.json", "ColorChecker amendment JSON (*.json)"}, ...
        "SpectraLab - Select unfinished ColorChecker amendment");
    if isequal(file, 0), return, end
    if isempty(regexp(string(file), ...
            "^colorchecker_session_amendment_\d{3}\.json$", "once"))
        error("SpectraLab:Examples:ColorCheckerAmendmentRequired", ...
            ["Select a file named " ...
             "colorchecker_session_amendment_NNN.json. A paused " ...
             "colorchecker_session.json belongs to the acquisition " ...
             "resume workflow."]);
    end
