...
def main() -> int:
    ...
    try:
        while True:
            startup_attempt += 1
            log_event(
                f"startup attempt {startup_attempt}/{max(1, args.startup_retries)}"
            )
            ...
            while True:
                ...
                if idx == 0:
                    ...
                    can_retry = (
                        ...
                        and not reading_taken
                        and startup_attempt < max(1, args.startup_retries)
                    )
    
                    if can_retry:
                        log_event(
                            f"startup communication failure: {failure_text}; "
                            f"raw={raw_path}; retrying"
                        )
                        print(
                            "spotread communication failed before calibration; "
